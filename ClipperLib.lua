...
--[[
  ...
  * Purpose   :  Lua wrapper for Clipper2 v1.5.3 DLL                             *
...
function Paths:__tostring()
  local out = {}
  for index = 1, self:size() do
    out[index] = ("Path %d: %s"):format(
      index, tostring(self:get(index)))
  end
  return table.concat(out, "\n")
end
...
local function to_lua_paths(cpaths)
  ...
  for _ = 1, count do
    ...
    local path = Path:new(npts); idx = idx + 2
    for _ = 1, npts do
      local px, py = cpaths[idx], cpaths[idx + 1]
      path:add(Point(px, py)); idx = idx + 2
    end
    result:add(path)
  end
  return result
...
---@param subj                Paths|nil # subject path (may be null)
...
---@return Paths              # result polygons
function Clipper:boolean(cliptype, fillrule, subj, clip,
...
function Clipper:intersect(subj, clip, fillrule, precision)
  return self:boolean(ClipType.Intersection, fillrule
    or FillRule.EvenOdd, subj, clip, precision)
end
...
function Clipper:union(subj, clip, fillrule, precision)
  return self:boolean(ClipType.Union, fillrule
    or FillRule.EvenOdd, subj, clip, precision)
end
...
function Clipper:difference(subj, clip, fillrule, precision)
  return self:boolean(ClipType.Difference, fillrule
    or FillRule.EvenOdd, subj, clip, precision)
end
...
function Clipper:xor(subj, clip, fillrule, precision)
  return self:boolean(ClipType.Xor, fillrule
    or FillRule.EvenOdd, subj, clip, precision)
end
...
function Clipper:inflate_path(path, delta, jointype, endtype,
  ...
  local out_solution = instance.InflatePathD(
    path_buf, delta, jointype, endtype, precision,
    miter_limit, arc_tolerance, reverse_solution)

  local result = to_lua_paths(out_solution)
  instance.DisposeArrayD(out_solution)
  return result
end
...
function Clipper:inflate_paths(paths, delta, jointype, endtype,
  ...
  local out_solution = instance.InflatePathsD(
...
function Clipper:rect_clip(rect, paths, precision)
  ...
  local out_sol = instance.RectClipD(crect, paths._buf_ref[1], precision)
  local result = to_lua_paths(out_sol)
  instance.DisposeArrayD(out_sol)
  return result
end
...
---@param precision? integer # decimal precision (default = 2)
---@return Paths     # result segments
function Clipper:rect_clip_lines(rect, paths, precision)
  local precision = precision or 2
  local crect = ffi.new("CRectD", rect.left, rect.top, rect.right, rect.bottom)
  local out_sol = instance.RectClipLinesD(crect, paths._buf_ref[1], precision)
  local result = to_lua_paths(out_sol)
  instance.DisposeArrayD(out_sol)
  return result
end
